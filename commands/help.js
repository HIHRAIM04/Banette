...
module.exports = {
	...
	execute(message, args) {
		...
		function allCommands(msg) {
			commands.relax = commands.filter(command => command.category == 'relax')
			commands.info = commands.filter(command => command.category == 'info')
			let commandsText = commands.info.map(c => `•\`${c.name}\``).join('\n') + '\n**Развлечения**\n' + commands.relax.map(c => `•\`${c.name}\``).join('\n')
			let helpEmbed = new Discord.MessageEmbed()
				.setTitle('Список команд')
				.setDescription('**Инфо**\n' + commandsText + `\n\nТы можешь отправить \`${prefix}help [имя команды]\` чтобы получить информацию о ней!`)
				.setColor('RANDOM')
			let options = [
				{
					label: 'Все команды',
